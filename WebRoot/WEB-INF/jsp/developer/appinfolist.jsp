	...
	<div class="col-md-12">
		<div class="x_panel">
			...
			<input type="hidden" name="path" value="${pageContext.request.contextPath }"/>
				<form method="post" action="${pageContext.request.contextPath }/dev/userList">
			    ...
			    <ul>
					...
					<li>
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="APPqueryStatus" class="form-control">
								 <option value="">--请选择--</option>
									<c:if test="${APPstatusList != null }">
									   <c:forEach var="dataDictionary" items="${APPstatusList}">
									   		<option <c:if test="${dataDictionary.valueId == APPqueryStatus }">selected="selected"</c:if>
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="queryStatus" class="form-control">
									<c:if test="${SSPTstatusList != null }">
									   <option value="">--请选择--</option>
									   <c:forEach var="dataDictionary" items="${SSPTstatusList}">
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="queryCategoryLevel1" id="queryCategoryLevel1" class="form-control">
									<c:if test="${listcategory1 != null }">
									   <option value="">--请选择--</option>
									   <c:forEach var="appCategory" items="${listcategory1}">
									   		<option <c:if test="${appCategory.parentId == appGroyOne }">selected="selected"</c:if>
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="queryCategoryLevel2" id="queryCategoryLevel2" class="form-control">
									<c:if test="${listcategory2 != null }">
									   <option value="">--请选择--</option>
									   <c:forEach var="appCategory" items="${listcategory2}">
									   		<option <c:if test="${appCategory.parentId == appGroyOne }">selected="selected"</c:if>
									   		value="${appCategory.id}">${appCategory.categoryName}</option>
									   </c:forEach>
									</c:if>
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="queryCategoryLevel3" id="queryCategoryLevel3" class="form-control">
									<c:if test="${listcategory3 != null }">
									   <option value="">--请选择--</option>
									   <c:forEach var="appCategory" items="${listcategory3}">
									   		<option <c:if test="${appCategory.parentId == appGroyOne }">selected="selected"</c:if>
									   		value="${appCategory.id}">${appCategory.categoryName}</option>
									   </c:forEach>
									</c:if>
				...
				<div class="row">
					...
					<a href="${pageContext.request.contextPath}/dev/appinfoadd" class="btn btn-success btn-sm">新增APP基础信息</a>
                      ...
                      <ul class="dropdown-menu" role="menu">
                        ...
                        <li><a class="addVersion" appinfoid="${appInfo.id }" name="appinfoid" value="${appInfo.id }" data-toggle="tooltip"  data-placement="top" title="" data-original-title="新增APP版本信息">新增版本</a>
                        </li>
                        ...
                        <li><a  href="${pageContext.request.contextPath }/dev/update/${appInfo.id }" class="modifyAppInfo" 
